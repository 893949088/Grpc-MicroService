...
//#endif
                                              ...
                                              ServiceRequest serviceRequest)
        {
            ...
            //webRequest.ContentLength = (userSetContentLength >= 0 && userSetContentLength <= streamLength) ? userSetContentLength : streamLength;
            ...
            {
                data.WriteTo(requestStream, (userSetContentLength >= 0 && userSetContentLength <= streamLength) ? userSetContentLength : streamLength);
    ...
    internal static class HttpFactory
    {
        ...
        {
            webRequest.ContinueTimeout = configuration.ConnectionTimeout;
            ...
            foreach (var h in serviceRequest.Headers)
            {
                ...
                //else
                webRequest.Headers[h.Key] = h.Value;
            }
            
            // Set user-agent
            if (!string.IsNullOrEmpty(configuration.UserAgent))
            {
                webRequest.Headers["User-agent"] = configuration.UserAgent;
            }
        }
